...
var fotki = INHERIT({}, {
    ...
    albums: {
        ...
        /**
         * Get collection of root albums and save it to fotki.albums._collectionCache.
         * Then fotki.albums.getByName() will take result only from this cache
         * and fotki.albums.create, fotki.albums.edit, fotki.albums.delete will update
         * this cache.
         * Use only if your app has exclusive access to user account.
         * @returns {Promise}
         */
        prepareCollectionCache: function(){
            var defer = VOW.defer();
            fotki.albums.getCollection().then(function(albums){
                fotki.albums._collectionCache = albums;
                defer.resolve();
            });
        ...
        getByName: function(name) {
            if(fotki.albums._collectionCache){
                var collectionDefer = VOW.defer(),
                    collectionPromise = collectionDefer.promise();
                collectionDefer.resolve(fotki.albums._collectionCache);
            }else{
                collectionPromise = fotki.albums.getCollection();
            }
            return collectionPromise.then(function(albums){
        ...
        delete: function(uri) {
            return fotki._makeRequest({
                ...
                if(fotki.albums._collectionCache){
                    fotki.albums._collectionCache = fotki.albums._collectionCache.filter(function(album){
                        return (uri !== album.link.self && uri !== album.link.alternate);
                    });
                }
                return result;
            });
